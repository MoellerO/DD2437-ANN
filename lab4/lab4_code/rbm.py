...
class RestrictedBoltzmannMachine:
    ...
    def cd1(self, visible_trainset, n_iterations=30001):
        ...
        for it in range(n_iterations):
            ...
            # TODO USE BINARY FOR HIDDEN NODES AND PROBS FOR VISIBLE NODES

            self.update_params(v_0, h_0, pv, h)
            ...
            if it % self.rf["period"] == 0 and self.is_bottom:
                viz_rf(
                    ...
                    it=it,
                    grid=self.rf["grid"],
                    nhidden=self.ndim_hidden,
                )

            # print progress

            if it % self.print_period == 0:
                _, hp = self.get_h_given_v(visible_trainset)
                _, vp = self.get_v_given_h(hp)
                recon_loss = (
                    np.sum(np.linalg.norm(vp - visible_trainset, axis=1))
    ...
    def update_params(self, v_0, h_0, v_k, h_k):
        ...
        # Add deepcopy?
        self.delta_bias_v += (
            self.learning_rate * (np.mean(v_0, axis=0) - np.mean(v_k, axis=0))
            - self.momentum * self.delta_bias_v
        )
        self.delta_weight_vh += (
            self.learning_rate
            * (
                np.mean(np.einsum("...i,...j", v_0, h_0), axis=0)
                - np.mean(np.einsum("...i,...j", v_k, h_k), axis=0)
            )
            - self.momentum * self.delta_weight_vh
        )
        self.delta_bias_h += (
            self.learning_rate * (np.mean(h_0, axis=0) - np.mean(h_k, axis=0))
    ...
    def get_h_given_v(self, visible_minibatch):
        ...
        p_h_given_v = sigmoid(
            self.bias_h + np.matmul(visible_minibatch, self.weight_vh)
        )  # Naive implementation
        # h_given_v = np.random.binomial(1, p_h_given_v, size=p_h_given_v.shape)
    ...
    def get_v_given_h(self, hidden_minibatch):
        ...
        if self.is_top:
            ...
            p_v_given_h = np.hstack(
                (p_v_given_h_rbm, p_v_given_h_lbl)
            )  # orientation? .T?
            ...
            p_v_given_h = sigmoid(
                self.bias_v + np.matmul(self.weight_vh, hidden_minibatch.T).T
            )  # Naive implementation, consider using weight_vh!
            # v_given_h = np.random.binomial(1, p_v_given_h, size=p_v_given_h.shape)
    ...
    def get_h_given_v_dir(self, visible_minibatch):
        ...
        p_h_given_v = sigmoid(
            self.bias_h + np.matmul(visible_minibatch, self.weight_v_to_h)
        )  # Naive implementation
        h_given_v = sample_binary(p_h_given_v)
    ...
    def get_v_given_h_dir(self, hidden_minibatch):
        ...
        if self.is_top:
            ...
            p_v_given_h = sigmoid(
                self.bias_v + np.matmul(self.weight_h_to_v.T, hidden_minibatch.T).T
            )  # Switch places, remove transposes?
            v_given_h = sample_binary(p_v_given_h)
